...
class WebScraper:
    def __init__(self, start_url, max_links=30, max_depth=10, max_threads=3, skip_embedding=False, collection_name="mindtickle_summaries"):
    ...
    def mark_url_processed(self, url):
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute('UPDATE url_metadata SET processed = 1 WHERE url = ?', (url,))
            self.conn.commit()
    ...
    def check_bulk_links(self, urls):
        ...
        try:
            ...
            self.logger.info(f"Checked {len(urls)} URLs. {len(already_in_db)} are already in the database.")
    ...
    def setup_chromadb(self, collection_name):
        try:
            ...
            self.client = chromadb.PersistentClient(path="./chroma_data")  # For persistent storage
            ...
            self.logger.error(f"An error occurred while initializing the ChromaDB client or collection: {e}")
    ...
    def load_queue(self):
        ...
        if os.path.exists(self.queue_file):
            ...
            self.logger.debug(f"No queue file found. Starting with a new queue.")
            return deque()
    ...
    def extract_links(self, html_content):
        ...
        try:
            ...
            print(f"Extracted {len(all_links)} links from content.here are all the links: {all_links}")
            links = [a['href'] for a in all_links if a['href'].startswith(self.start_url)]
            # Extract all links that dont already exist in the database
            if links:
                not_valid_links, all_links = self.check_bulk_links(links)
                self.logger.debug(f"Extracted links_that_already_exist: {not_valid_links}, here are all the links that dont: {all_links}")
            else:
                self.logger.debug(f"Extracted 0 links from content.here are all the links: {all_links}")

            # Extract all valid links which belong to the domain
            
            self.logger.debug(f"Extracted {len(links)} links from content.here are all the links: {all_links}")
            return links
        except Exception as e:
        ...
        try:
            ...
            if self.skip_embedding or embedding:
                self.collection.add(
                    ...
                    metadatas=[{"title": title, "content": content, "url": url, "content_hash": content_hash, "depth": depth}],
                    ids=[url]
                )
    ...
    def comprehensive_crawler_threaded(self):
        ...
        if not self.queue:
            ...
            self.logger.info(f"Resuming with a queue of size: {len(self.queue)}, here is the current queue: {self.queue}")

        with ThreadPoolExecutor(self.max_threads) as executor:
            futures = {executor.submit(self.process_url, url, depth): (url, depth) for url, depth in self.queue}
            while futures:
                    ...
                    try:
                        ...
                        if result:
                            ...
                            if self.link_count < self.max_links and depth + 1 <= self.max_depth:
                                ...
                                for link in links:
                                    if link not in self.visited and self.link_count < self.max_links:
            ...
            if not self.queue and os.path.exists(self.queue_file):
                ...
                self.logger.info(f"Crawl completed. Queue file '{self.queue_file}' has been deleted.")
        ...
        try:
            ...
            while self.queue and self.link_count < self.max_links:
                ...
                if self.link_count < self.max_links and depth + 1 <= self.max_depth:
                    ...
                    for link in links:
                        if link not in self.visited and not self.is_url_visited(link) and self.link_count < self.max_links:
                            self.queue.append((link, depth + 1))
                        else:
